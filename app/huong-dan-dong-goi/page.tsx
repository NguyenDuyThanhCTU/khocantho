import { Metadata } from "next";
import { PackageSearch } from "lucide-react";
import PackagingContent from "@/components/StaticPages/PackagingContent";

export const metadata: Metadata = {
  title: "Hướng Dẫn Đóng Gói Hàng Gửi Quốc Tế Chuẩn | An Tâm Express",
  description:
    "Cẩm nang hướng dẫn đóng gói an toàn cho các mặt hàng đặc thù: mắm, cá khô, nệm Kymdan, hàng dễ vỡ, mỹ phẩm. Giúp tiết kiệm cước thể tích và thông quan dễ dàng.",
  keywords:
    "đóng gói gửi hàng đi Mỹ, cách gửi mắm đi Úc, ép chân không nệm Kymdan, đóng gói gửi hàng quốc tế",
};

export default function PackagingGuidePage() {
  return (
    <main className="min-h-screen bg-slate-50">
      {/* --- HERO SECTION --- */}
      <section className="relative pt-32 pb-48 bg-[#0A192F] overflow-hidden">
        {/* Background Network Pattern */}
        <div className="absolute inset-0 z-0 pointer-events-none">
          <div className="absolute inset-0 bg-[url('/images/dots-pattern.svg')] opacity-[0.05] invert" />
          <div className="absolute top-[-20%] left-[-10%] w-[600px] h-[600px] bg-brand-orange/15 rounded-full blur-[150px]" />
          <div className="absolute bottom-[-10%] right-[-10%] w-[500px] h-[500px] bg-sky-500/10 rounded-full blur-[120px]" />
        </div>

        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
          <div className="inline-flex items-center justify-center gap-2 px-4 py-2 rounded-full bg-white/5 text-brand-orange font-bold text-sm uppercase tracking-wider mb-6 border border-white/10 backdrop-blur-md">
            <PackageSearch className="w-4 h-4" /> An toàn & Tiết kiệm
          </div>
          <h1 className="text-4xl md:text-6xl font-extrabold text-white leading-tight mb-6">
            Quy chuẩn đóng gói <br className="hidden md:block" />
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-brand-orange to-yellow-500">
              Vận tải Hàng không
            </span>
          </h1>
          <p className="text-lg text-slate-300 max-w-2xl mx-auto">
            Hàng hóa được bảo vệ tối đa qua hệ thống hút chân không và vật liệu
            chống sốc chuyên dụng. Đảm bảo kiện hàng của bạn an toàn tuyệt đối
            trong suốt hành trình.
          </p>
        </div>
      </section>

      {/* --- INTERACTIVE PACKAGING CONTENT --- */}
      <PackagingContent />
    </main>
  );
}
